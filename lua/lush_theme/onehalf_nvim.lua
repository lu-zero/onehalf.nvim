...
if vim.g.onehalf_override ~= nil then
  local override = vim.g.onehalf_override
...
if vim.g.onehalf_settings ~= nil then
  if vim.g.onehalf_settings.bold == false then
    bold_strings = "NONE"
  end
  if not vim.g.onehalf_settings.italic == false then
    italic_strings = "NONE"
  end
  if not vim.g.onehalf_settings.underline == false then
...
--     Debug          {}, --    debugging statements
    ...
    TSPunctBracket       {fg = fg1}, -- For brackets and parens.
